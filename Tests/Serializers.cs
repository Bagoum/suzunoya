...
public class Serializers {
    ...
    public abstract class Trait {
        public class Glowing : Trait {
            public static Glowing S { get; } = new();
        }
        public class Warm : Trait {
            public static Warm S { get; } = new();
        }
        public class Cool : Trait {
            public static Cool S { get; } = new();
        }
    }
    
    [JsonConverter(typeof(SingletonConverter<Trait2>))]
    public abstract class Trait2 {
        public static string SINGLETON_KEY => nameof(Glowing.Singleton);
        public class Glowing : Trait2 {
            public static Glowing Singleton { get; } = new();
        }
        public class Warm : Trait2 {
            public static Warm Singleton { get; } = new();
        }
        public class Cool : Trait2 {
            public static Cool Singleton { get; } = new();
        }
    }

    [Test]
    public void TestSingletonSerialize() {
        Trait[] traits = [new Trait.Glowing(), new Trait.Warm(), new Trait.Cool()];
        
        var typs = new JsonSerializerSettings() {TypeNameHandling = TypeNameHandling.Auto};
        string s1 = JsonConvert.SerializeObject(traits, Formatting.None, typs);
        Assert.AreEqual("[\"Glowing\",\"Warm\",\"Cool\"]", s1);
        var deserialized = JsonConvert.DeserializeObject<Trait[]>(s1)!;
        Assert.AreSame(deserialized[0], Trait.Glowing.S);
        Assert.AreSame(deserialized[1], Trait.Warm.S);
        Assert.AreSame(deserialized[2], Trait.Cool.S);
    }
    
    [Test]
    public void TestSingletonSerializeCustomKey() {
        Trait2[] traits = [new Trait2.Glowing(), new Trait2.Warm(), new Trait2.Cool()];
        
        var typs = new JsonSerializerSettings() {TypeNameHandling = TypeNameHandling.Auto};
        string s1 = JsonConvert.SerializeObject(traits, Formatting.None, typs);
        Assert.AreEqual("[\"Glowing\",\"Warm\",\"Cool\"]", s1);
        var deserialized = JsonConvert.DeserializeObject<Trait2[]>(s1)!;
        Assert.AreSame(deserialized[0], Trait2.Glowing.Singleton);
        Assert.AreSame(deserialized[1], Trait2.Warm.Singleton);
        Assert.AreSame(deserialized[2], Trait2.Cool.Singleton);
