...
public class ParentingTests {
    ...
    public void ParentingTest1() {
        ...
        parent.LocalLocation.Value = new(2, 3, 0);
        var child = vn.Add(new Yukari());
        child.LocalLocation.Value = new(1, 6, 0);
        child.Parent = parent;
        Assert.AreEqual(child.ComputedLocation.Value, new Vector3(3, 9, 0));
        parent.LocalLocation.Value = new(3, 2, 0);
        ...
        child.Parent = parent;
        child.LocalLocation.Value = new(2, 5, 0);
        Assert.AreEqual(child.ComputedLocation.Value, new Vector3(5, 7, 0));
        parent.Delete();
        Assert.AreEqual(child.EntityActive.Value, EntityState.Deleted);
    }
}
}