...
public abstract record AST(PositionRange Position, LexicalScope EnclosingScope, params IAST[] Params) {
    ...
    /// <inheritdoc cref="IMethodTypeTree{T}.WillSelectOverload"/>
    public virtual Either<Unifier, TypeUnifyErr> WillSelectOverload(InvokedMethod _, IImplicitTypeConverterInstance? cast, Unifier u) {
    ...
    /// <inheritdoc cref="IAtomicTypeTree.WillSelectOverload"/>
    ...
    /// <inheritdoc cref="IAST.SetDiagnostics"/>
    public void SetDiagnostics(ReflectDiagnostic[] diagnostics) {
        this.Diagnostics = diagnostics;
    }
    
    /// <inheritdoc cref="IAST.WarnUsage"/>
    ...
    /// <inheritdoc cref="ToSemanticTokens"/>
    ...
    /// </summary>
    public IEnumerable<DocumentSymbol> FlattenParams(Func<IAST, int, DocumentSymbol>? mapper) {
        bool DefaultFlatten(IAST ast) => ast is Array 
                && ast.SelectedOverloadReturnType?.Resolve(Unifier.Empty).LeftOrNull?.GetElementType() is {} rt 
    ...
    public record WeakReference : AST, IAST, IAtomicTypeTree {
        ...
        public override TEx _RealizeWithoutCast(TExArgCtx tac) {
            ...
            return t.MakeTypedTEx(LexicalScope.DynamicVariableLookup(tac, tac.EnvFrame, Name, t));
        }
        ...
        public TEx RealizeAsWeakWriteable(TExArgCtx tac, Func<Ex, Ex> opOnValue) {
            var t = ReturnType(SelectedOverload!);
            return t.MakeTypedTEx(LexicalScope.DynamicVariableLookup(tac, tac.EnvFrame, Name, t, null, opOnValue));
        }
    ...
    public record TypeAs(PositionRange Position, LexicalScope EnclosingScope, Type CastToTyp, IAST Body) :
        ...
        public override TEx _RealizeWithoutCast(TExArgCtx tac) {
            ...
            try {
                //nested compiles cause issues with baking
                var conv = (tac.Ctx.BakeTracker is ExBakeTracker.None ?
                                ExHelpers.TryFlattenConversion(body, CastToTyp) : null) 
                           ?? Ex.Convert(body, CastToTyp);
    ...
    public record MethodCall(PositionRange Position, PositionRange MethodPosition,
        ...
        public IReadOnlyList<ITypeTree> Arguments => Params;
        /// <inheritdoc/>
        public List<InvokedMethod>? RealizableOverloads { get; set; }
        /// <inheritdoc/>
        public (InvokedMethod method, Dummy simplified)? SelectedOverload { get; set; }
        /// <inheritdoc/>
        ...
        void IMethodTypeTree<InvokedMethod>.GenerateOverloads(List<(TypeDesignation, Unifier)>[] arguments) {
            if (!specialIntMethods.HasValue) return;
            /* Multiply op is problematic.
             * We can't query for all defined arithmetic operators in netstandard2.1 (it's possible in net7),
             * so we define custom overloads for multiplication:
             * (float,float)->float, (int,int)->int, (float,T)->T, (T,float)->T. These cover most cases.
             * However, if we get an (int * float) expression, this must be parsed using (float,float) implicit cast
             *  instead of (T,float), which is incorrect.
             */
            var (all, intOnly) = specialIntMethods.Value;
            foreach (var arg in arguments)
                foreach (var (td, _) in arg)
                    //Probably don't need to handle Variable+RestrTypes
                    // as float*float should be preferred if int isn't strongly known
                    if (td is Known k && k.Typ == typeof(int)) {
                        Methods = intOnly;
                        return;
                    }
            Methods = all;
        ...
        private static STAnnotater? MaybeMakeLocalScope(STAnnotater enclosing, PositionRange methodPos, InvokedMethod[] methods) {
            ...
            if (nWithScope > 0) {
                ...
                ServiceLocator.Find<ILangCustomizer>().Declare(sc, methodPos, cfg.type);
                return enclosing with { Scope = sc };
            } else if (methods.All(m => m.Mi.GetAttribute<ExtendsInternalScopeAttribute>() != null)) {
                ServiceLocator.Find<ILangCustomizer>().Extend(enclosing.Scope, methodPos, 
                    methods[0].Mi.GetAttribute<ExtendsInternalScopeAttribute>()!.type, null);
            }
            return null;
        }

        /// <inheritdoc cref="IMethodTypeTree{Dummy}.ImplicitParameterCast"/>
        public virtual Either<IImplicitTypeConverter, bool> ImplicitParameterCast(InvokedMethod overload, int index) {
            if (EnclosingScope.GlobalRoot.GetConverterForCompiledExpressionType(overload.Mi.Params[index].Type) is { } conv)
                return new(conv);
        ...
        /// <inheritdoc cref="IMethodTypeTree{T}.WillSelectOverload" />
        public override Either<Unifier, TypeUnifyErr> WillSelectOverload(InvokedMethod mi, IImplicitTypeConverterInstance? cast, Unifier u) {
            AllowInvokeAsConst = mi.Mi.Flags.HasFlag(MethodFlags.ConstableAny)
                                 || (mi.Mi.Flags.HasFlag(MethodFlags.ConstableNonAOT) && 
                                     ServiceLocator.Find<ILangCustomizer>().AOTMode is AOTMode.None);
            if (!AllowInvokeAsConst && (mi.Mi.GetAttribute<ConstableAttribute>() ??
        ...
        public static TEx RealizeMethod(IAST? ast, IMethodSignature mi, TExArgCtx tac, Func<int, TExArgCtx, TEx> prmGetter, bool allowInvokeAsConst=false) {
            ...
            if (mi.ReturnType.IsTExOrTExFuncType(out var dataTyp)) {
                ...
                var invoked = allowInvokeAsConst ?
                    mi.InvokeExIfNotConstant(prms) :
                    mi.InvokeEx(prms);
                var retTyp = mi.ReturnType;
                var baseTyp = MethodSignature.CheckHiddenReturnType(retTyp);
                if (baseTyp != retTyp) {
                    if (invoked is ConstantExpression ce)
                        invoked = Ex.Constant(ce.Value, baseTyp);
                    return baseTyp.MakeTypedTEx(invoked);
                } else
                    return retTyp.MakeTypedTEx(invoked);
            }
        }
            ...
            return AddMethodSemanticToken ?
                children.Prepend(
                    SelectedOverload is { } m ?
                        ServiceLocator.Find<ILangCustomizer>()
                            .FromMethod(m.method.Mi, MethodPosition, retType: m.simplified.Resolve().LeftOrNull) :
    ...
    public record InstanceMethodCall(PositionRange Position, PositionRange MethodPosition, LexicalScope EnclosingScope, string Name, params IAST[] Params) : MethodCall(Position, MethodPosition, EnclosingScope, [], Params), IMethodTypeTree<InvokedMethod> {
        ...
        void IMethodTypeTree<InvokedMethod>.GenerateOverloads(List<(TypeDesignation, Unifier)>[] arguments) {
            var meths = (Arg0PossibleTypes = arguments[0]).SelectMany(tu => {
                ...
                return t.GetMember(Name).Where(x => x.GetCustomAttribute<DontReflectAttribute>() is null)
                    ...
                    .Concat(Scope.GlobalRoot.ExtensionMethods(t, Name));
            }).Where(sig => (!sig.Member.Static || sig.Member is TypeMember.Method { IsExtension: true })
                            && sig.Params.Length == Params.Length);
            if (GenericTypes.Length == 0) {
                meths = meths.Where(m => m is not GenericMethodSignature);
            } else {
                meths = meths.SelectNotNull(m => {
                    if (m is not GenericMethodSignature gm)
                        return null;
                    if (gm.TypeParams != GenericTypes.Length)
                        return null;
                    return gm.Specialize(GenericTypes);
                });
            }
            Methods = meths
        ...
        /// <inheritdoc/>
        public override Either<IImplicitTypeConverter, bool> ImplicitParameterCast(InvokedMethod overload, int index) {
            if (index == 0)
                return false;
    ...
    public record PartialMethodCall : AST, IMethodAST<PartialInvokedMethod> {
        private InvokedMethod[] Methods { get; }
        /// <inheritdoc/>
        ...
        protected override IEnumerable<SemanticToken> _ToSemanticTokens() =>
            base._ToSemanticTokens().Prepend(
                SelectedOverload is { } m ?
                    ServiceLocator.Find<ILangCustomizer>().FromMethod(m.method.Meth.Mi, MethodPosition, 
    ...
    public record PartialScriptFunctionCall(PositionRange Position, PositionRange MethodPosition, LexicalScope EnclosingScope,
        ...
        public override TEx _RealizeWithoutCast(TExArgCtx tac) {
            ...
            var target = Definition.IsConstant ?
                PartialFn.PartiallyApply(Definition.Compile(tac), prms) :
                PartialFn.PartiallyApply(Definition.Compile(tac), prms.Prepend(raisedEf));
    ...
    public record ScriptFunctionCall(PositionRange Position, PositionRange MethodPosition,
        ...
        Either<IImplicitTypeConverter, bool> IMethodTypeTree<Dummy>.ImplicitParameterCast(Dummy overload, int index) {
            if (overload.Arguments[index].Resolve().TryL(out var t) && 
                EnclosingScope.GlobalRoot.GetConverterForCompiledExpressionType(t) is { } conv)
                return new(conv);
            return true;
        }
        ...
        public override TEx _RealizeWithoutCast(TExArgCtx tac) {
            ...
            if (Definition.IsConstant) {
                var func = Definition.Compile(tac);
                ...
                target = PartialFn.Execute(Definition.Compile(tac), prms.Prepend(raisedEf));
            end:
            return ReturnType(SelectedOverload?.simplified!).MakeTypedTEx(target);
        }
    ...
    public record ScriptFunctionDef(PositionRange Position, string Name, LexicalScope EnclosingScope, LexicalScope FnScope, ScriptFnDecl Definition, Block Body) : AST(Position, EnclosingScope, Definition.Defaults.FilterNone().Append(Body).ToArray()), IMethodAST<Dummy> {
        ...
        public object CompileFunc(Type fnType) {
            ...
            Func<TExArgCtx, TEx> body = tac => {
                tac.Ctx.CompileAsScriptFn = Definition;
                return Ex.Block(
                    Body.Realize(tac),
            ...
            var func = ServiceLocator.Find<ILangCustomizer>().CompileDelegate(fnType, body, argsWithEf);
            return func;
        }
    ...
    public record Return(PositionRange Position, LexicalScope EnclosingScope, IAST? Value) : AST(Position,
        ...
        /// <inheritdoc cref="AST.ReplaceScope"/>
    ...
    public record Block : AST, IMethodAST<Dummy> {
        ...
        public void FinalizeUnifiers(Unifier unifier) {
            ...
            ServiceLocator.Find<ILangCustomizer>().AttachEFToLambdaLikeObject(this, unifier);
        }
        ...
        public static Ex MakeExpressionBlock(AST me, LexicalScope localScope, TExArgCtx tac, Func<TExArgCtx, IEnumerable<Expression>> stmts) {
            ...
            if (localScope.UseEF) {
                ...
                if (tac.MaybeGetByType<EnvFrame>(out _) is { } tex) {
                    if (localScope.Parent is GlobalScope) {
    ...
    public record Array : AST, IMethodAST<Dummy> {
        ...
        public Array(PositionRange Position, LexicalScope EnclosingScope, Type? EleTyp, params IAST[] Params) : base(Position,
            ...
            if (EleTyp is null)
                ...
                ElementType = TypeDesignation.FromType(EleTyp);
    ...
    public record Number : AST, IAST, IAtomicTypeTree, IAnyTypedValueAST {
        ...
        public override TEx _RealizeWithoutCast(TExArgCtx tac) {
            ...
            if (typ == typeof(int))
                return (TEx<int>)Ex.Constant((int)System.Math.Round(Value));
