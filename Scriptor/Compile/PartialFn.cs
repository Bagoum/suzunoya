...
public static class PartialFn {
    ...
    public static Ex Execute(Ex func, IEnumerable<Ex> args, bool allowConst = false) {
        ...
        if (!allowConst || ServiceLocator.Find<ILangCustomizer>().AOTMode is not AOTMode.None) 
            goto deflt;
    ...
    public static Ex PartiallyApply(Ex func, IEnumerable<Ex> _applied) {
        ...
        var delTypes = origFnTypes.Take(applied.Length) //A, B
            .Prepend(func.Type) //Func<A,B,C,D...R>
            .Append(ReflectionUtils.MakeFuncType(origFnTypes.Skip(applied.Length).ToArray())) //Func<C,D...R>
            .ToArray();
        ...
        var helper = ServiceLocator.Find<ILangCustomizer>().CompileDelegate(delFnType, body, args);
