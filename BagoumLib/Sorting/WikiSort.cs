...
internal struct RangeSE {
	...
	public RangeSE(int start1, int end1) {
		start = start1;
		end = end1;
	}
...
struct Iterator {
	...
	public RangeSE nextRange() {
		...
		return new RangeSE(start, decima);
	}
...
public class WikiSorter<T> {
	...
	int BinaryFirst(T[] array, T value, RangeSE range, IComparer<T> comp) {
	...
	int BinaryLast(T[] array, T value, RangeSE range, IComparer<T> comp) {
		int start = range.start, end = range.end - 1;
		while (start < end) {
			int mid = start + (end - start)/2;
	...
	int FindFirstForward(T[] array, T value, RangeSE range, IComparer<T> comp, int unique) {
		...
		for (index = range.start + skip; comp.Compare(array[index - 1], value) < 0; index += skip)
			if (index >= range.end - skip)
				return BinaryFirst(array, value, new RangeSE(index, range.end), comp);
		
		return BinaryFirst(array, value, new RangeSE(index - skip, index), comp);
	}
	
	int FindLastForward(T[] array, T value, RangeSE range, IComparer<T> comp, int unique) {
		if (range.length() == 0) return range.start;
		int index, skip = Math.Max(range.length()/unique, 1);
		
		for (index = range.start + skip; comp.Compare(value, array[index - 1]) >= 0; index += skip)
			if (index >= range.end - skip)
				return BinaryLast(array, value, new RangeSE(index, range.end), comp);
		
		return BinaryLast(array, value, new RangeSE(index - skip, index), comp);
	}
	
	int FindFirstBackward(T[] array, T value, RangeSE range, IComparer<T> comp, int unique) {
		if (range.length() == 0) return range.start;
		int index, skip = Math.Max(range.length()/unique, 1);
		
		for (index = range.end - skip; index > range.start && comp.Compare(array[index - 1], value) >= 0; index -= skip)
			if (index < range.start + skip)
				return BinaryFirst(array, value, new RangeSE(range.start, index), comp);
		
		return BinaryFirst(array, value, new RangeSE(index, index + skip), comp);
	}
	
	int FindLastBackward(T[] array, T value, RangeSE range, IComparer<T> comp, int unique) {
		if (range.length() == 0) return range.start;
		int index, skip = Math.Max(range.length()/unique, 1);
		
		for (index = range.end - skip; index > range.start && comp.Compare(value, array[index - 1]) < 0; index -= skip)
			if (index < range.start + skip)
				return BinaryLast(array, value, new RangeSE(range.start, index), comp);
		
		return BinaryLast(array, value, new RangeSE(index, index + skip), comp);
	}
	
	// n^2 sorting algorithm used to sort tiny chunks of the full array
	void InsertionSort(T[] array, RangeSE range, IComparer<T> comp) {
	...
	void Reverse(T[] array, RangeSE range) {
	...
	void Rotate(T[] array, int amount, RangeSE range, bool use_cache) {
		...
		RangeSE range1 = new RangeSE(range.start, split);
		RangeSE range2 = new RangeSE(split, range.end);
	...
	void MergeInto(T[] from, RangeSE A, RangeSE B, IComparer<T> comp, T[] into, int at_index) {
		int A_index = A.start;
		int B_index = B.start;
	...
	void MergeExternal(T[] array, RangeSE A, RangeSE B, IComparer<T> comp) {
	...
	void MergeInternal(T[] array, RangeSE A, RangeSE B, IComparer<T> comp, RangeSE buffer) {
			...
			while (true) {
				if (comp.Compare(array[B.start + B_count], array[buffer.start + A_count]) >= 0) {
					(array[A.start + insert], array[buffer.start + A_count]) = 
						(array[buffer.start + A_count], array[A.start + insert]);
					A_count++;
					insert++;
					if (A_count >= A.length()) break;
				} else {
					(array[A.start + insert], array[B.start + B_count]) = 
						(array[B.start + B_count], array[A.start + insert]);
					B_count++;
					insert++;
	...
	void MergeInPlace(T[] array, RangeSE A, RangeSE B, IComparer<T> comp) {
		...
		A = new RangeSE(A.start, A.end);
		B = new RangeSE(B.start, B.end);
		...
		while (true) {
			...
			Rotate(array, -amount, new RangeSE(A.start, mid), true);
			if (B.end == mid) break;
	...
	void NetSwap(T[] array, int[] order, RangeSE range, IComparer<T> comp, int x, int y) {
		int compare = comp.Compare(array[range.start + x], array[range.start + y]);
		if (compare > 0 || (order[x] > order[y] && compare == 0)) {
			(array[range.start + x], array[range.start + y]) = (array[range.start + y], array[range.start + x]);
			(order[x], order[y]) = (order[y], order[x]);
		}
	}
	
	// bottom-up merge sort combined with an in-place merge algorithm for O(1) memory use
	public void Sort(T[] array, IComparer<T> comp) {
		...
		while (!iterator.finished()) {
			...
			RangeSE range = iterator.nextRange();
		...
		RangeSE buffer1 = new RangeSE(), buffer2 = new RangeSE();
		RangeSE blockA = new RangeSE(), blockB = new RangeSE();
		RangeSE lastA = new RangeSE(), lastB = new RangeSE();
		RangeSE firstA = new RangeSE();
		RangeSE A = new RangeSE(), B = new RangeSE();

		Pull pull0 = new();
		Pull pull1 = new();
		ref Pull currPull = ref pull0;
		bool isPull1 = false;
			...
			if (iterator.length() < cache_size) {
				...
				if ((iterator.length() + 1) * 4 <= cache_size && iterator.length() * 4 <= size) {
					...
					while (!iterator.finished()) {
						...
						RangeSE A1 = iterator.nextRange();
						RangeSE B1 = iterator.nextRange();
						RangeSE A2 = iterator.nextRange();
						RangeSE B2 = iterator.nextRange();
						...
						RangeSE A3 = new RangeSE(0, A1.length());
						RangeSE B3 = new RangeSE(A1.length(), A1.length() + A2.length());
					...
					while (!iterator.finished()) {
						...
						if (comp.Compare(array[B.end - 1], array[A.start]) < 0) {
							...
							Rotate(array, A.length(), new RangeSE(A.start, B.end), true);
				...
				while (!iterator.finished()) {
					...
					for (last = A.start, count = 1; count < find; last = index, count++) {
						index = FindLastForward(array, array[last], new RangeSE(last + 1, A.end), comp, find - count);
					...
					if (count >= buffer_size) {
						...
						currPull.range.set(A.start, B.end);
						currPull.count = count;
						currPull.from = index;
						currPull.to = A.start;
						currPull = ref pull1;
						isPull1 = true;
						...
						if (count == buffer_size + buffer_size) {
							...
							buffer1 = new RangeSE(A.start, A.start + count);
					...
					} else if (!isPull1 && count > buffer1.length()) {
					...
					for (last = B.end - 1, count = 1; count < find; last = index - 1, count++) {
						index = FindFirstBackward(array, array[last], new RangeSE(B.start, last), comp, find - count);
					...
					if (count >= buffer_size) {
						...
						if (count == buffer_size + buffer_size) {
							...
							// found one buffer, but now find the other one
							buffer1 = new RangeSE(B.end - count, B.end);
							find_separately = false;
						} else {
							// buffer2 will be pulled out from a 'B' subarray, so if the first buffer was pulled out from the corresponding 'A' subarray,
							// we need to adjust the end point for that A subarray so it knows to stop redistributing its values before reaching buffer2
							if (pull0.range.start == A.start) pull0.range.end -= pull1.count;
				...
				void Modify(ref Pull p) {
					...
					if (p.to < p.from) {
						...
						for (count = 1; count < length; count++) {
							index = FindFirstBackward(array, array[index - 1], new RangeSE(p.to, p.from - (count - 1)), comp, length - count);
							RangeSE range = new RangeSE(index + 1, p.from + 1);
						...
						index = p.from + 1;
						for (count = 1; count < length; count++) {
							index = FindLastForward(array, array[index], new RangeSE(index, p.to), comp, length - count);
							RangeSE range = new RangeSE(p.from, index - 1);
				...
				while (!iterator.finished()) {
					...
					int start = A.start;
					if (start == pull0.range.start) {
						if (pull0.from > pull0.to) {
							A.start += pull0.count;
						...
						} else if (pull0.from < pull0.to) {
							B.end -= pull0.count;
							if (B.length() == 0) continue;
						}
					}
					if (start == pull1.range.start) {
						if (pull1.from > pull1.to) {
							A.start += pull1.count;
							if (A.length() == 0) continue;
						} else if (pull1.from < pull1.to) {
							B.end -= pull1.count;
							if (B.length() == 0) continue;
						}
					}
					...
					if (comp.Compare(array[B.end - 1], array[A.start]) < 0) {
						...
						if (blockA.length() > 0) {
							while (true) {
								...
								if ((lastB.length() > 0 && comp.Compare(array[lastB.end - 1], array[indexA]) >= 0) || blockB.length() == 0) {
									...
									(array[blockA.start], array[indexA]) = (array[indexA], array[blockA.start]);
									indexA++;
									...
									if (lastA.length() <= cache_size)
										MergeExternal(array, lastA, new RangeSE(lastA.end, B_split), comp);
									else if (buffer2.length() > 0)
										MergeInternal(array, lastA, new RangeSE(lastA.end, B_split), comp, buffer2);
									else
										MergeInPlace(array, lastA, new RangeSE(lastA.end, B_split), comp);
									...
									if (buffer2.length() > 0 || block_size <= cache_size) {
										...
										Rotate(array, blockA.start - B_split, new RangeSE(B_split, blockA.start + block_size), true);
									}
									...
									Rotate(array, -blockB.length(), new RangeSE(blockA.start, blockB.end), false);
						...
						if (lastA.length() <= cache_size)
							MergeExternal(array, lastA, new RangeSE(lastA.end, B.end), comp);
						else if (buffer2.length() > 0)
							MergeInternal(array, lastA, new RangeSE(lastA.end, B.end), comp, buffer2);
						else
							MergeInPlace(array, lastA, new RangeSE(lastA.end, B.end), comp);
					}
				}
				...
				void Modify2(ref Pull p) {
					int unique = p.count * 2;
					if (p.from > p.to) {
						// the values were pulled out to the left, so redistribute them back to the right
						RangeSE buffer = new RangeSE(p.range.start, p.range.start + p.count);
						while (buffer.length() > 0) {
							index = FindFirstForward(array, array[buffer.start], new RangeSE(buffer.end, p.range.end), comp, unique);
							int amount = index - buffer.end;
							Rotate(array, buffer.length(), new RangeSE(buffer.start, index), true);
						...
						RangeSE buffer = new RangeSE(p.range.end - p.count, p.range.end);
						while (buffer.length() > 0) {
							index = FindLastBackward(array, array[buffer.end - 1], new RangeSE(p.range.start, buffer.start), comp, unique);
							int amount = buffer.start - index;
							Rotate(array, amount, new RangeSE(index, buffer.end), true);
							buffer.start -= amount;
							buffer.end -= (amount + 1);
							unique -= 2;
						}
	...
	public void Sort(T[] array, int start, int end, IComparer<T> comp) => Sort(array, comp);
}
